...
// BeginUseCase handles stream start operations
type BeginUseCase interface {
...
type beginUseCaseImpl struct {
	repo   repository.ConnStateRepository
	crypto service.CryptoService
	sender service.CellSenderService
}

// NewBeginUseCase creates a new begin use case
func NewBeginUseCase(repo repository.ConnStateRepository, crypto service.CryptoService, sender service.CellSenderService) BeginUseCase {
	return &beginUseCaseImpl{
		repo:   repo,
		crypto: crypto,
		sender: sender,
	}
...
func (uc *beginUseCaseImpl) Begin(st *entity.ConnState, cid vo.CircuitID, cell *entity.Cell, ensureServeDown func(*entity.ConnState)) error {
	nonce := st.BeginNonce()
...
func (uc *beginUseCaseImpl) forwardUpstream(st *entity.ConnState, cid vo.CircuitID, sid vo.StreamID, down net.Conn) {
	defer down.Close()
