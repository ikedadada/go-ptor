...
func TestDataUseCase_DataExit(t *testing.T) {
	repo := repoimpl.NewConnStateRepository(time.Second)
	crypto := service.NewCryptoService()
	cellSender := service.NewCellSenderService()
	uc := usecase.NewDataUseCase(repo, crypto, cellSender)
...
func TestDataUseCase_DataHidden(t *testing.T) {
	repo := repoimpl.NewConnStateRepository(time.Second)
	crypto := service.NewCryptoService()
	cellSender := service.NewCellSenderService()
	uc := usecase.NewDataUseCase(repo, crypto, cellSender)
...
func TestDataUseCase_DataUpstream(t *testing.T) {
	repo := repoimpl.NewConnStateRepository(time.Second)
	crypto := service.NewCryptoService()
	cellSender := service.NewCellSenderService()
	uc := usecase.NewDataUseCase(repo, crypto, cellSender)
