...
func TestEndStreamUseCase_EndStreamSpecific(t *testing.T) {
	repo := repoimpl.NewConnStateRepository(time.Second)
	cellSender := service.NewCellSenderService()
	uc := usecase.NewEndStreamUseCase(repo, cellSender)
...
func TestEndStreamUseCase_EndAllStreams(t *testing.T) {
	repo := repoimpl.NewConnStateRepository(time.Second)
	cellSender := service.NewCellSenderService()
	uc := usecase.NewEndStreamUseCase(repo, cellSender)
...
func TestEndStreamUseCase_EndStreamWithForward(t *testing.T) {
	repo := repoimpl.NewConnStateRepository(time.Second)
	cellSender := service.NewCellSenderService()
	uc := usecase.NewEndStreamUseCase(repo, cellSender)
