...
// ExtendUseCase handles circuit extension operations
type ExtendUseCase interface {
...
// NewExtendUseCase creates a new extend use case
func NewExtendUseCase(priv vo.PrivateKey, repo repository.ConnStateRepository, crypto service.CryptoService, sender service.CellSenderService) ExtendUseCase {
	return &extendUseCaseImpl{
		priv:   priv,
		repo:   repo,
		crypto: crypto,
		sender: sender,
...
func (uc *extendUseCaseImpl) Extend(up net.Conn, cid vo.CircuitID, cell *entity.Cell) error {
...
func (uc *extendUseCaseImpl) ForwardExtend(st *entity.ConnState, cid vo.CircuitID, cell *entity.Cell) error {
	if st.Down() == nil {
